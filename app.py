...
def sign_in():
    ...
    result = db.users.find_one({'userid': username_receive, 'password': pw_hash})
...
def main():
    ...
    lists = list(db.plan.find({ },{'_id':False}).sort("_id", -1).limit(4))
    # posts에 memodb list data를 담은 lists를 연결한다. 이 posts는 main.html에서 jinja2 engine으로 사용될 예정이다.
    return render_template('main.html', posts = lists, user_info = user_info)
...
def insert_memo():
    ...
    doc = {'id':user_info['userid'], 'plan':memo_receive}
    # doc안에 있는 data를 mongodb에 추가한다.
    db.plan.insert_one(doc)
...
def delete_memo():
    ...
    db.plan.delete_one({'plan': memo_receive})
